...
class FakeEmployeeTrackingHubService extends EmployeeTrackingHubService {
  FakeEmployeeTrackingHubService() : super(storageService: FakeSecureStorage());
  ...
  void emitStatusChange(IncidentStatusUpdate update) =>
      _statusController.add(update);
  void emitLocationUpdate(TeamLocationUpdate update) =>
...
void main() {
  group('EmployeeTrackingBloc Unit Tests', () {
    ...
    test(
      'emits EmployeeTrackingLoaded when LoadMyIncidents succeeds',
      () async {
    ...
    test(
      'updates team coordinates and calculates route when TeamLocationReceived',
      () async {
    ...
    test('disables editing when incident transitions to Resolved', () async {
      ...
      bloc.add(
        const IncidentStatusReceived(incidentId: 'inc-1', status: 'Resolved'),
      );
