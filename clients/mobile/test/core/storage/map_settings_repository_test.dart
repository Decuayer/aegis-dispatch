...
void main() {
  ...
  group('MapSettingsRepository Unit Tests', () {
    test(
      'isBoundaryLockEnabled returns true by default when storage is empty',
      () async {
    ...
    test(
      'isBoundaryLockEnabled returns stored boolean value when set to false',
      () async {
        SharedPreferences.setMockInitialValues({
    ...
    test(
      'setBoundaryLockEnabled updates preference in local storage',
      () async {
        SharedPreferences.setMockInitialValues({});
        final prefs = await SharedPreferences.getInstance();
        final repository = MapSettingsRepository(prefs: prefs);
