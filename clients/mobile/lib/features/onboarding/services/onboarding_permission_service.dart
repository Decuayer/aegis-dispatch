...
enum OnboardingPermissionResult {
  granted,
  denied,
  permanentlyDenied,
}
...
class OnboardingPermissionService {
  ...
  const OnboardingPermissionService({
    PermissionHandlerService permissionHandlerService = const PermissionHandlerService(),
  ...
  Future<OnboardingPermissionResult> requestOnboardingPermissions() async {
    ...
    final foregroundGranted = await _permissionHandlerService.requestForegroundLocationPermission();
    if (!foregroundGranted) {
      final isPermanentlyDenied = await _permissionHandlerService.isLocationPermanentlyDenied();
