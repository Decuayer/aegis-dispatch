...
class _GoogleAccountTileState extends State<GoogleAccountTile> {
  ...
  Future<void> _linkGoogleAccount() async {
    ...
    try {
      ...
      if (!mounted) return;
      await context.read<ProfileCubit>().linkGoogleAccount(widget.user, idToken);
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Google bağlantı hatası: ${e.toString().replaceAll('Exception: ', '')}'),
  ...
  Future<void> _unlinkGoogleAccount() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Bağlantı kaldırılamadı: ${e.toString().replaceAll('Exception: ', '')}'),
          backgroundColor: AppColors.error,
          behavior: SnackBarBehavior.floating,
        ),
      );
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(
                  isLinked ? Icons.check_circle_outline_rounded : Icons.link_rounded,
          ...
          if (isLinked)
            ...
            OutlinedButton.icon(
              ...
              icon: busy
                  ...
                  : const Icon(Icons.g_mobiledata_rounded, size: 28, color: AppColors.primaryDark),
              label: Text(
