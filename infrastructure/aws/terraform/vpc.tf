...
# 4. Private Subnets (ECS Fargate Tasks)
...
# 6. Elastic IP and NAT Gateway for Private Subnet Outbound Access
...
resource "aws_security_group" "ecs_sg" {
  ...
  description = "Allow ingress strictly from ALB target group to ECS Fargate"
  vpc_id      = aws_vpc.main.id
  ...
  egress {
    description = "Allow all outbound for external services"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
...
resource "aws_security_group" "redis_sg" {
  ...
  description = "Allow Redis traffic strictly from ECS tasks"
  vpc_id      = aws_vpc.main.id
